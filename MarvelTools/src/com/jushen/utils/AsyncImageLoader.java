 ...
 * 
 */
public class AsyncImageLoader extends Controller{
...
//		}
	...
	protected void onDownloadWithUrl(String vImageUrl){
		// 内存和文件中都没有再从网络下载
		if (!TextUtils.isEmpty(vImageUrl)) {
			new ImageDownloadTask().execute(vImageUrl);
		}
	}
...
//				}
				...
				LoggerUtils.e("download " + imageUrl + " failed");
			}
			onDownloadFinish(imageUrl, result);
		}
	...
	private File getDiskCacheDir(Context context, String uniqueName) {
		...
		if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
			...
			if(CacheDir == null){
				Log.w("TAg", "getExternalCacheDir can not find, try getCacheDir ");
				CacheDir = context.getCacheDir();
				if(CacheDir == null)
					Log.e("TAg", "shit ! getCacheDir can not find");
			}
		} else {
			CacheDir = context.getCacheDir();
		}
